...
func _get_selected_amount():
	var amount = 0
	for r in _responses:
		if (r as String).contains("una garrrr"):
			amount = 1
		elif (r as String).contains("tres garrrr"):
			amount = 3
		elif (r as String).contains("cinco garrr"):
			amount = 5
		elif (r as String).contains("monedas de oro garrrrr"):
			amount = -1
	return amount
